...
import Loading from "../loading/Loading"; // 로딩 컴포넌트 임포트
...
const VoiceInfo = ({ file }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (file) {
            formData.append("file", file); // MP3 파일 추가
        }
        formData.append("mno", mno); // mno는 정수형으로 전송
        formData.append("start", 1); // start는 정수형으로 전송
        formData.append("end", 1); // end는 정수형으로 전송
